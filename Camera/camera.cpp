...
void Camera::setParamResolution()
{
    ...
    // this->cam.camera_width = firstFrame.cols;
    // this->cam.camera_height = firstFrame.rows;
...
{
    this->cam.loadCalibration("/home/helloworld/szy/single_cam/parameters/" + this->camera_name + ".yaml");
    while ( false == open() ) {
        LOG(ERROR) << "迈德相机打开失败...";
...
bool MVCamera::open()
{
    ...
    //设置曝光
    if ( -1 != this->cam.exposure ) {
        MV_CHECK_API_WARNING(CameraSetAeState(hCamera, false))  // 取消自动曝光
        MV_CHECK_API_WARNING(CameraSetExposureTime(hCamera, this->cam.exposure))
    }

    // 设置增益 
    if ( -1 != this->cam.rGain && -1 != this->cam.gGain && -1 != this->cam.bGain)
        MV_CHECK_API_WARNING(CameraSetGain(hCamera, this->cam.rGain, this->cam.gGain, this->cam.bGain))
    if ( -1 != this->cam.analogGain )
        MV_CHECK_API_WARNING(CameraSetAnalogGain(hCamera, this->cam.analogGain))
    if ( -1 != this->cam.gamma )
        MV_CHECK_API_WARNING(CameraSetGamma(hCamera, this->cam.gamma))
    ...
    // 畸变矫正设置
    // double cameraMatrix[] = {this->cam.fx, this->cam.fy, this->cam.cx, this->cam.cy};
    // double distCoeff[ ] = {this->cam.k1, this->cam.k2, this->cam.p1, this->cam.p2, this->cam.k3};
